...
export async function action({ request }: Route.ActionArgs) {
  ...
  try {
    ...
    // 2. INTERCEPT: Grab the raw HttpOnly cookie string generated by Spring Boot
    ...
    // 3. Process backend user details
    const user = await backendResponse.json();
    if (!user.accountNonLocked) {
      throw new Error("This account has been administratively suspended.");
    }

    // 4. SAVE: Store the Spring Boot cookie inside our secure frontend server vault
    const frontendCookieString =
      await serverSessionService.createSession(springCookie);

    // 5. Append our frontend encrypted session string to the browser redirect
